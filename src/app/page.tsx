...
export default function Home() {
  ...
  useEffect(() => {
    const handler = (e: KeyboardEvent) => {
      ...
      if (e.key === 'Escape') {
        ...
        if (hasFocus) { dispatch({ type: 'SET_FOCUS_TASK', payload: null }); e.preventDefault(); return; }
  ...
  }, [state.taskModal, state.commandPaletteOpen, state.focusTaskId, state.viewMode, state.theme, showStats, sidebarOpen, dispatch, openTaskModal]);
        ...
        <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/50" onClick={() => setShowShortcuts(false)}>
          ...
          >
            <div className="flex items-center justify-between px-5 py-4 border-b" style={{ borderColor: 'var(--color-border)' }}>
              <h2 className="text-lg font-semibold" style={{ color: 'var(--color-text-primary)' }}>Keyboard Shortcuts</h2>
              <button onClick={() => setShowShortcuts(false)} className="p-1 rounded-lg hover:opacity-70" style={{ color: 'var(--color-text-tertiary)' }}>
                <X size={18} />
              </button>
            </div>
            <div className="px-5 py-4 max-h-[70vh] overflow-y-auto space-y-5">
              {[
                ...
                { title: 'In Task Modal', shortcuts: [
                  { keys: ['⌘', '↵'], desc: 'Save task' },
                  { keys: ['Esc'], desc: 'Close modal' },
                ]},
              ].map(section => (
                <div key={section.title}>
                  <h3 className="text-xs font-semibold uppercase tracking-wider mb-2" style={{ color: 'var(--color-text-tertiary)' }}>{section.title}</h3>
                    ...
                    {section.shortcuts.map(s => (
                      <div key={s.desc} className="flex items-center justify-between py-1">
                        ...
                        <div className="flex items-center gap-1">
                          {s.keys.map((k, i) => (
                            <kbd key={i} className="px-2 py-0.5 rounded text-xs font-mono min-w-6 text-center"
