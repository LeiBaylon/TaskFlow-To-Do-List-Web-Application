  ...
  const contextMenuItems: MenuEntry[] = [
    ...
    },
    {
      ...
      children: COLUMNS.map((col) => ({
        ...
        label: col.label,
        disabled:
          (task.completed && col.id === "done") ||
          (!task.completed && task.status === col.id),
        action: () => {
          updateTask(task.id, {
      ...
      >
        <div
          ...
          style={{
            ...
            boxShadow:
              isSelected ?
                "0 0 0 1px var(--color-accent), 0 2px 8px rgba(99,102,241,0.15)"
              : "none",
          }}
        >
...
function DroppableColumn({
  ...
  return (
    <div
      ...
      style={{
        background:
          isOver ? "var(--color-surface-hover)" : "var(--color-background)",
        border:
          isOver ? "2px dashed var(--color-accent)" : "2px dashed transparent",
      }}
    >
      {children}
    </div>
...
export default function KanbanBoard({
  ...
  const columnTasks = useMemo(() => {
    ...
    return {
      ...
      "in-progress": tasks.filter(
        (t) => t.status === "in-progress" && !t.completed,
      ),
  ...
  const handleQuickAdd = (status: Task["status"]) => {
    openTaskModal({
      mode: "add",
      defaults: { status, folderId: state.activeFolderId },
    });
  };
