...
import {
  Check, Flag, Calendar, Clock, ChevronRight, ChevronDown,
  Trash2, GripVertical, Play, MoreHorizontal, Tag,
  Pencil, FolderOpen, Plus, ArrowRightLeft,
} from 'lucide-react';
import { useApp } from '@/store/AppContext';
import { formatRelativeDate, getPriorityColor, getPriorityLabel } from '@/lib/nlp';
...
export default function TaskItem({ task, depth = 0 }: TaskItemProps) {
  ...
  const contextMenuItems = useMemo((): MenuEntry[] => {
    ...
    items.push(
      ...
      },
      {
        ...
        children: [
          { id: 'status-todo', label: 'To Do', action: () => { updateTask(task.id, { status: 'todo', completed: false, completedAt: null }); closeMenu(); } },
          { id: 'status-in-progress', label: 'In Progress', action: () => { updateTask(task.id, { status: 'in-progress', completed: false, completedAt: null }); closeMenu(); } },
          { id: 'status-done', label: 'Done', action: () => { updateTask(task.id, { status: 'done', completed: true, completedAt: new Date().toISOString() }); closeMenu(); } },
        ],
      },
      {
        id: 'move',
    ...
    items.push(
      { id: 'div2', type: 'divider' },
      {
        ...
        icon: <Plus size={14} />,
        action: () => {
          openTaskModal({ mode: 'add', defaults: { parentId: task.id, folderId: task.folderId } });
          setIsExpanded(true);
          closeMenu();
        },
      },
      {
        ...
        action: () => {
          ...
          const duplicateWithSubtasks = (t: Task, newParentId: string | null) => {
            const newId = addTask({
              title: t.title,
              completed: false,
              priority: t.priority,
    ...
    return items;
  }, [task, state.folders, closeMenu, handleToggle, startFocus, updateTask, deleteTask, addTask, openTaskModal, setIsExpanded]);
  ...
  return (
    <div style={{ paddingLeft: depth * 24, opacity: isDragging ? 0.4 : 1 }}>
      ...
      >
        {/* Swipe backgrounds */}
        <motion.div className="absolute inset-0 rounded-xl flex items-center px-4"
          style={{ background: bgLeft }}>
          <motion.div style={{ opacity: checkOpacity }}>
            <Check size={18} className="text-emerald-500" />
          </motion.div>
        </motion.div>
        <motion.div className="absolute inset-0 rounded-xl flex items-center justify-end px-4"
        ...
        >
          ...
          {/* Subtask expand */}
          {hasSubtasks ? (
            <button onClick={() => setIsExpanded(!isExpanded)} className="pt-0.5">
          ...
          <button
            ...
            className="relative mt-0.5 w-[18px] h-[18px] rounded-full border-2 flex items-center justify-center shrink-0 transition-all duration-200"
            style={{
          ...
          <div className="flex-1 min-w-0">
            {isEditing ? (
              ...
              <p className={`text-sm font-medium leading-snug transition-all duration-200 ${task.completed ? 'line-through opacity-40' : ''}`}
                style={{ color: 'var(--color-text-primary)' }}>
                {task.title}
              </p>
            )}
            ...
            <div className="flex items-center gap-2 mt-1 flex-wrap">
              {task.dueDate && (
                <span className="inline-flex items-center gap-1 text-[11px]"
                  style={{ color: new Date(task.dueDate) < new Date() && !task.completed ? 'var(--color-danger)' : 'var(--color-text-tertiary)' }}>
                  <Calendar size={10} />
                  {formatRelativeDate(task.dueDate)}
                  {task.dueTime && <><Clock size={10} className="ml-0.5" />{task.dueTime}</>}
                </span>
              )}
              {task.priority < 4 && (
              ...
              {task.tags.map(tag => (
                <span key={tag} className="text-[11px] px-1.5 py-0 rounded-full"
            ...
            {hasSubtasks && (
              <div className="mt-1.5 w-full max-w-[120px] h-1 rounded-full overflow-hidden"
          ...
          <AnimatePresence>
            {showActions && !task.completed && (
              ...
              >
                <button onClick={startFocus} className="p-1.5 rounded-lg hover:bg-black/5 dark:hover:bg-white/5 transition-colors"
                  title="Focus mode">
                  <Play size={13} style={{ color: 'var(--color-text-tertiary)' }} />
                </button>
                <button onClick={() => deleteTask(task.id)} className="p-1.5 rounded-lg hover:bg-red-50 dark:hover:bg-red-900/20 transition-colors"
                  title="Delete">
