...
import {
  ChevronLeft, ChevronRight, Plus, Flag, Clock,
  Check, Trash2, CalendarDays, Pencil, Play, FolderOpen, ArrowRightLeft,
} from 'lucide-react';
  ...
  return (
    <>
      <motion.div
        ...
        style={{
          background: task.completed ? 'var(--color-surface-hover)' : `${getPriorityColor(task.priority)}12`,
          color: task.completed ? 'var(--color-text-tertiary)' : getPriorityColor(task.priority),
        }}
      ...
      >
        ...
        >
          {task.completed && <Check size={7} className="text-white" strokeWidth={3} />}
        </button>
        ...
        <button
          ...
          className="ml-auto opacity-0 group-hover/chip:opacity-60 hover:!opacity-100 transition-opacity flex-shrink-0"
        >
          <Trash2 size={9} />
        </button>
      </motion.div>
...
function DayDetail({ date, tasks, onClose }: {
  ...
  const handleToggle = (id: string, wasCompleted: boolean) => {
    if (!wasCompleted) {
      confetti({
        particleCount: 30, spread: 50, origin: { y: 0.65 },
  ...
  return (
    ...
    >
      ...
      <div className="flex items-center justify-between mb-3">
        ...
        <button onClick={onClose} className="text-xs px-2 py-1 rounded-lg transition-colors"
      ...
      {/* Add task button */}
      <button
        onClick={() => openTaskModal({ mode: 'add', defaults: { dueDate: toDateKey(date), folderId: state.activeFolderId } })}
      ...
      <div className="space-y-1.5 max-h-[40vh] overflow-y-auto">
            ...
            >
              ...
              <div className="flex-1 min-w-0">
                ...
                <div className="flex items-center gap-1.5 mt-0.5">
                  ...
                  {task.priority < 4 && (
                    <span className="text-[10px]" style={{ color: getPriorityColor(task.priority) }}>
                      <Flag size={8} className="inline" /> {getPriorityLabel(task.priority)}
                    </span>
                  )}
                </div>
              </div>
        ...
        {completed.length > 0 && (
          <>
            <p className="text-[10px] font-medium uppercase tracking-wider pt-2 px-1"
              style={{ color: 'var(--color-text-tertiary)' }}>
            ...
            {completed.map(task => (
              ...
              >
                ...
                <p className="text-xs font-medium truncate line-through"
                  style={{ color: 'var(--color-text-tertiary)' }}>
                  {task.title}
                </p>
              </motion.div>
...
export default function CalendarView() {
  ...
  const { menu: dayMenu, handleContextMenu: handleDayContextMenu, closeMenu: closeDayMenu } = useContextMenu();
  ...
  const tasksByDate = useMemo(() => {
    ...
    state.tasks
      .filter(t => t.folderId === state.activeFolderId && !t.parentId && t.dueDate)
      .forEach(t => {
        const key = t.dueDate!;
        if (!map[key]) map[key] = [];
        map[key].push(t);
    ...
    return [
      {
        ...
        icon: <Plus size={14} />,
        action: () => {
          openTaskModal({ mode: 'add', defaults: { dueDate: dateKey, folderId: state.activeFolderId } });
          closeDayMenu();
        },
      },
      {
  ...
  return (
    <div className="max-w-4xl mx-auto w-full">
      ...
      <div className="flex gap-5 items-start flex-col lg:flex-row">
        ...
        <div className="flex-1 min-w-0">
          ...
          <div className="flex items-center gap-3 mb-4">
            <button onClick={goToPrev} className="p-1.5 rounded-lg transition-colors hover:bg-black/5 dark:hover:bg-white/5"
            ...
            <button onClick={goToNext} className="p-1.5 rounded-lg transition-colors hover:bg-black/5 dark:hover:bg-white/5"
              style={{ color: 'var(--color-text-secondary)' }}>
                ...
                return (
                  ...
                  >
                        ...
                        <span className="text-[9px] font-medium px-1 rounded"
                          ...
                          {completedCount > 0 && (
                            <span style={{ color: 'var(--color-success)' }}>
                              {incompleteTasks.length > 0 ? '+' : ''}{completedCount}✓
                            </span>
                          )}
                        </span>
                      )}
                    ...
                    <div className="space-y-0.5">
                      ...
                      {dayTasks.length > 3 && (
                        <p className="text-[9px] font-medium px-1"
                          style={{ color: 'var(--color-text-tertiary)' }}>
      ...
      <AnimatePresence>
        {dayMenu && (
          <ContextMenu x={dayMenu.x} y={dayMenu.y} items={dayContextMenuItems} onClose={closeDayMenu} />
        )}
      </AnimatePresence>
    </div>
  );
