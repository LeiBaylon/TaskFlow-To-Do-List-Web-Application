...
const DEFAULT_QUOTES = [
  ...
  { id: '3', text: 'Do what you can, with what you have, where you are.', author: 'Theodore Roosevelt' },
  { id: '4', text: 'Small daily improvements are the key to staggering long-term results.', author: '' },
  { id: '5', text: 'Focus on being productive instead of busy.', author: 'Tim Ferriss' },
  { id: '6', text: 'You don\'t have to be great to start, but you have to start to be great.', author: 'Zig Ziglar' },
];
...
export default function QuotesCarousel() {
  ...
  return (
    <div
      ...
      onMouseLeave={() => { if (!editingId && !adding) setPaused(false); }}
    >
      ...
      <div className="absolute top-3 right-3 flex items-center gap-1">
        ...
        <button
          onClick={() => { setAdding(true); setPaused(true); }}
          className="p-1.5 rounded-md hover:opacity-70 transition-opacity"
          style={{ color: 'var(--color-text-tertiary)' }}
          title="Add quote"
        >
            ...
            {editingId === current.id ? (
              ...
              >
                ...
                <div className="flex items-center gap-2">
                  <button onClick={saveEdit} className="p-1 rounded-md hover:opacity-80" style={{ color: 'var(--color-success)' }}>
                    <Check size={14} />
                  </button>
                  <button onClick={cancelEdit} className="p-1 rounded-md hover:opacity-80" style={{ color: 'var(--color-text-tertiary)' }}>
                    <X size={14} />
                  </button>
          ...
          >
            <div className="space-y-2 pt-2 border-t" style={{ borderColor: 'var(--color-border)' }}>
              ...
              <div className="flex gap-2">
                ...
                <button
                  onClick={() => { setAdding(false); setNewText(''); setNewAuthor(''); }}
      ...
      <div className="flex items-center justify-center gap-2">
        <button onClick={prev} className="p-1 rounded-md hover:opacity-70 transition-opacity" style={{ color: 'var(--color-text-tertiary)' }}>
          <ChevronLeft size={14} />
        </button>
        <div className="flex gap-1">
          {quotes.map((_, i) => (
            <button
              ...
              style={{ background: i === currentIndex ? 'var(--color-accent)' : 'var(--color-border)' }}
            />
          ))}
        </div>
        <button onClick={next} className="p-1 rounded-md hover:opacity-70 transition-opacity" style={{ color: 'var(--color-text-tertiary)' }}>
